...
// Anchored, edge-to-edge scenic weather backgrounds for each workability level.
// Rendered at very low opacity (~12%) so scheduled job cards stay readable.

// A row of grass blades anchored along the bottom edge.
function GrassRow({ color, bent = false }: { color: string; bent?: boolean }) {
  const blades = Array.from({ length: 14 }, (_, i) => i);
  return (
    <svg
      viewBox="0 0 140 20"
    ...
    >
      {blades.map((i) => {
        const x = i * 10 + 3;
        const lean = bent ? 5 : i % 2 === 0 ? 1.5 : -1.5;
        return (
          <path
            key={i}
            d={`M${x} 20 Q ${x + lean} 9 ${x + lean * 2} 2`}
            stroke={color}
            strokeWidth={2}
            strokeLinecap="round"
            fill="none"
...
function WorkabilityIllustration({ level }: { level: WorkabilityLevel }) {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden opacity-[0.13]">
      {level === 1 && (
        <>
          {/* Sun anchored to top-right corner */}
          <svg viewBox="0 0 60 60" className="absolute -right-2 -top-2 h-16 w-16" aria-hidden>
          ...
          {/* Two sunflowers growing from the grass */}
          <svg viewBox="0 0 100 60" preserveAspectRatio="none" className="absolute bottom-4 left-4 h-12 w-10" aria-hidden>
            <line x1="20" y1="60" x2="20" y2="22" stroke="#16a34a" strokeWidth={3} />
            <circle cx="20" cy="16" r="7" fill="#f59e0b" />
            <circle cx="20" cy="16" r="3" fill="#92400e" />
            <line x1="60" y1="60" x2="60" y2="30" stroke="#16a34a" strokeWidth={3} />
            <circle cx="60" cy="24" r="6" fill="#f59e0b" />
            <circle cx="60" cy="24" r="2.5" fill="#92400e" />
          </svg>
          <GrassRow color="#16a34a" />
        </>
      )}

      {level === 2 && (
        <>
          {/* Fluffy clouds rolling in from the top-right */}
          <svg viewBox="0 0 80 40" className="absolute -right-1 top-1 h-14 w-24" aria-hidden>
            <g fill="#0ea5e9">
              <circle cx="30" cy="22" r="10" />
              <circle cx="44" cy="18" r="13" />
              <circle cx="60" cy="22" r="10" />
              <rect x="30" y="22" width="30" height="10" rx="5" />
            </g>
          </svg>
          <GrassRow color="#0284c7" />
        </>
      )}

      {level === 3 && (
        <>
          {/* Heavy clouds anchored to the top */}
          <svg viewBox="0 0 120 36" preserveAspectRatio="none" className="absolute inset-x-0 top-0 h-12 w-full" aria-hidden>
            <g fill="#d97706">
              <circle cx="30" cy="22" r="13" />
              <circle cx="55" cy="16" r="16" />
              <circle cx="82" cy="20" r="14" />
              <rect x="20" y="22" width="80" height="12" rx="6" />
            </g>
          </svg>
          {/* Slanting rain lines across the cell */}
          ...
          {/* Wind-bent grass */}
          <GrassRow color="#b45309" bent />
        </>
      )}

      {level === 4 && (
        <>
          {/* Dark storm clouds anchored to the top edge */}
          <svg viewBox="0 0 120 40" preserveAspectRatio="none" className="absolute inset-x-0 top-0 h-14 w-full" aria-hidden>
            <g fill="#9f1239">
              <circle cx="28" cy="24" r="15" />
              <circle cx="58" cy="16" r="19" />
              <circle cx="90" cy="22" r="16" />
              <rect x="18" y="24" width="84" height="14" rx="7" />
            </g>
          </svg>
          {/* Stylized lightning bolt striking downward */}
          <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" aria-hidden>
            <path
              d="M54 34 L40 62 L52 62 L44 84 L66 52 L54 52 L62 34 Z"
              fill="#eab308"
            />
          </svg>
          <GrassRow color="#9f1239" />
        </>
      )}
    </div>
  );
              ...
              {monthDays.map((d, i) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "relative min-h-[104px] border-b border-r border-border p-1.5 transition-colors [&:nth-child(7n)]:border-r-0",
                      !inMonth && "bg-muted/30",
                      f && WORKABILITY_META[f.level].tint,
